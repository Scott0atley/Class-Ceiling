...
cd "G:\Stata data and do\BHPS + US\UKDA-6614-stata\stata\stata13_se\ukhls_wx" 

merge m:m pidp using xwavedat.dta

codebook pasoc10_cc panssec8_dv masoc10_cc manssec8_dv jbnssec8_dv jbsoc10_cc fimnlabgrs_dv dvage sex racel plbornc hiqual_dv health sf1 gor_dv jbiindb_dv jbsect jbsectpub jbsize jbft_dv, compact
...
* Step 1: Sort data by id and wave
sort pidp Wave

* Step 2: Create a new variable (or update nssecdom if already populated)
gen nssecdom_updated = nssecdom

* Step 3: Carry forward the last observation within each id
bysort pidp (Wave): replace nssecdom_updated = nssecdom_updated[_n-1] if missing(nssecdom_updated)

* Step 4: Verify the results
tab nssecdom_updated

* Semi-dominance approach adds about 6,000 cases *

* age

gen birth = istrtdaty-birthy
...
replace white=0 if ethnic>1 & ethnic<=8
...
* Step 2: Create a new variable (or update nssecdom if already populated)
gen genhealth_updated = genhealth

* Step 3: Carry forward the last observation within each id
bysort pidp (Wave): replace genhealth_updated = genhealth_updated[_n-1] if missing(genhealth_updated)

* Step 4: Verify the results
tab genhealth_updated
...
replace industry=.m if(jbiindb_dv==0)
...
* Step 2: Create a new variable (or update nssecdom if already populated)
gen industry_updated = industry

* Step 3: Carry forward the last observation within each id
bysort pidp (Wave): replace industry_updated = industry_updated[_n-1] if missing(industry_updated)

* Step 4: Verify the results
tab industry_updated
lab values industry_updated industry_lbl
...
gen adjincome =.
replace adjincome = yearlyincome*100.2/62.9 if istrtdaty==1992 & istrtdatm>=1 & istrtdatm<=8
...
gen logincome = log(adjincome)



*Current NS-SEC and SOC codes
...
misstable summarize logincome size sector industry gor_dv genhealth hiqual_dv labhours gender white nage nage2 nssecdom_updated

misstable patterns logincome size sector industry gor_dv genhealth hiqual_dv labhours gender white nage nage2 nssecdom_updated

* Codebook of new variables

*gen health and industry are the biggest issues here....

codebook logincome size sector industry gor_dv genhealth hiqual_dv labhours gender white nage nage2 nssecdom_updated, compact

keep if !missing(logincome, size, sector, industry, gor_dv, genhealth, hiqual_dv, labhours, gender, white, nage, nage2, nssecdom)

regress logincome ib3.(nssecdom_updated) nage nage2 i.gender i.white labhours i.hiqual_dv i.gor_dv i.sector i.size i.Wave

xtreg logincome ib3.(nssecdom_updated) nage nage2 i.gender i.white labhours i.hiqual_dv i.genhealth i.gor_dv i.industry i.sector i.size



mixed logincome ib3.(nssecdom_updated) nage nage2 i.gender i.white labhours i.hiqual_dv i.gor_dv i.sector i.size i.Wave || pidp: || jbsoc00_cc: 

estat icc

predict re*, reffects

twoway scatter re1 re2, ylabel(, angle(horizontal))

histogram re1, normal title("Random Slopes for jbsoc00_cc")

sort pidp
line re1 re2, sort title("Random Slopes for jbsoc00_cc")
...
egen pickone = tag(jbsoc00_cc)
egen urank = rank(u) if pickone==1
serrbar u use urank, scale(1.96) 

sort urank 

list urank jbsoc00_cc u if pickone==1



*CCA*

misstable summarize adjincome nssecdom nage nage2 ethnic sex healthy genhealth hed labhours gor_dv curnssec sector industry size jbsoc00_cc istrtdaty Wave

misstable patterns adjincome nssecdom nage nage2 ethnic sex healthy genhealth hed labhours gor_dv curnssec sector industry size jbsoc00_cc istrtdaty Wave
...
egen miss1=rmiss(adjincome nssecdom nage nage2 ethnic sex healthy genhealth labhours gor_dv curnssec sector industry size jbsoc00_cc Wave)
tab miss1
keep if miss1==0
...
regress adjincome i.classorigin i.istrtdaty i.sex ib(8).gor_dv nage nage2
